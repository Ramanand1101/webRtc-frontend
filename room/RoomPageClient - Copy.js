...
function VideoBox({ stream, name, isCameraOff }) {
  ...
  useEffect(() => {
    ...
    //   }
    // }, [stream, isCameraOff]);
    if (isCameraOff || !stream) {
      videoEl.srcObject = null;
    } else {
      console.log("camera", isCameraOff)
      videoEl.srcObject = stream;
      console.log("sfff", stream)
    }
  }, [stream, isCameraOff]);
    ...
    <div className={style.videoBlock}>
      ...
      {!isCameraOff && stream ? (
        ...
        <div className={style.videoPlaceholder}>
          <Image src="/switch-off.png" alt="Camera Off" width={64} height={64} />
          <p>Camera Off</p>
        </div>
      )}
    </div>
...
export default function RoomPage() {
  ...
  useEffect(() => {
    ...
    const createPeer = (peerId) => {
      ...
      console.log("Sdsfsdfdsf", stream)
      if (stream) {
        stream.getTracks().forEach((track) => {
          if (track.readyState === 'live') {
            console.log("dfdff", track, stream)
    ...
    const start = async () => {
      ...
      socket.on('user-connected', async ({ socketId, name }) => {
        ...
        // ✅ Host sends offer to every new user using shared createPeer
        if (role === 'host' && streamRef.current) {
          ...
          console.log('📡 Host sent offer to:', socketId);
        }
      });
